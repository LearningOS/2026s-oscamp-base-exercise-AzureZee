...
impl Sv39PageTable {
    ...
    ///
    /// 提示：右移 (12 + level * 9) 位，然后与 0x1FF((1<<9)-1) 做掩码。
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        // 在 level 0 写入最终的物理页号和 flags
        let vpn0 = Sv39PageTable::extract_vpn(va, 0);
        let node = self.nodes.get_mut(&current_ppn).unwrap();
        let ppn = pa >> 12; // 物理页号
        node.entries[vpn0] = (ppn << PPN_SHIFT) | flags;
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for level in [2, 1, 0] {
            ...
            if (pte & (PTE_R | PTE_W | PTE_X)) != 0 {
                ...
                // 根据层级确定 offset 位数：
                // level 2 大页（1GB）：offset = 30 位（VPN[1] + VPN[0] + 12位）
                // level 1 大页（2MB）：offset = 21 位（VPN[0] + 12位）
                // level 0 普通页（4KB）：offset = 12 位
                let offset_mask = (1u64 << (12 + level * 9)) - 1;
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let vpn_l2 = Sv39PageTable::extract_vpn(va, 2);
        let pte = self.nodes.get(&current_ppn).unwrap().entries[vpn_l2];

        // 如果 PTE 无效，分配新的页表节点
        if (pte & PTE_V) == 0 {
            let new_ppn = self.alloc_node();
            let node = self.nodes.get_mut(&current_ppn).unwrap();
            node.entries[vpn_l2] = (new_ppn << PPN_SHIFT) | PTE_V;
            current_ppn = new_ppn;
        } else {
            // PTE 有效，提取 PPN 进入下一级
            current_ppn = pte >> PPN_SHIFT;
        ...
        // 在 level 1 写入最终的物理页号和 flags
        let vpn_l1 = Sv39PageTable::extract_vpn(va, 1);
        let node = self.nodes.get_mut(&current_ppn).unwrap();
        let ppn = pa >> 12; // 物理页号
        node.entries[vpn_l1] = (ppn << PPN_SHIFT) | flags;
    }
}
