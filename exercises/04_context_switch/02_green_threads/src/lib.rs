...
impl Scheduler {
    ...
    fn schedule_next(&mut self) {
        ...
        let next = (current + 1..threads.len())
            .chain(0..=current)
            .find(|&i| threads[i].state == ThreadState::Ready)
            .expect("no ready thread");
        // update current state
        if threads[current].state != ThreadState::Finished {
            threads[current].state = ThreadState::Ready;
        }
        self.current = next;
        // mark next running
        threads[next].state = ThreadState::Running;
        // if first schedule, set CURRENT_THREAD_ENTRY
        if let Some(entry) = threads[next].entry.take() {
            unsafe { CURRENT_THREAD_ENTRY = Some(entry) };
        }
        // switch context
        let (old, new) = unsafe {
            let old = &mut (&mut (*SCHEDULER)).threads[current].ctx;
            let new = &(&(*SCHEDULER)).threads[next].ctx;
            (old.as_mut_ptr(), new.as_ptr())
        };
