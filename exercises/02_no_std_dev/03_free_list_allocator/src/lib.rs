...
unsafe impl GlobalAlloc for FreeListAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        let alloc_end = align_up(alloc_end, align);
        if alloc_end > self.heap_end {
            null_mut()
        } else {
            let _ = self.bump_next.compare_exchange(
                current,
                alloc_end,
                Ordering::SeqCst,
                Ordering::SeqCst,
            );
            let ptr = self.bump_next.load(Ordering::SeqCst);
            ptr as *mut u8
        }
    }
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        ...
        let ptr: *mut FreeBlock = ptr.cast();
        let curr = self.free_list_head();
        let free_block = FreeBlock { size, next: curr };
        ptr.write(free_block);
